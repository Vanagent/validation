# Vanagent Satya Proof of Contribution
...
```bash
docker run --rm --volume $(pwd)/input:/input --volume $(pwd)/output:/output my-proof
```
...
```bash
docker run --rm --volume /path/to/input:/input --volume /path/to/output:/output my-proof
```
